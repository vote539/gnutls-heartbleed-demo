...
int main(void)
{
  ...
  gnutls_global_init();
	...
	gnutls::certificate_credentials credentials;
        ...
        /* connect to the peer
         */
	std::cout << "Spot A" << std::endl;
        sd = tcp_connect();
        session.set_transport_ptr((gnutls_transport_ptr_t) (ptrdiff_t)sd);

	gnutls_datum_t out;
	gnutls_certificate_type_t type = session.get_certificate_type();
	std::cout << "Spot A1" << std::endl;
	unsigned int status;
	int ret;
	session.verify_peers_certificate(status);
	std::cout << "Spot A2" << std::endl;
	ret = gnutls_certificate_verification_status_print(status, type, &out, 0);
	std::cout << "Status Print " << ret << std::endl;
	std::cout << "Spot B" << std::endl;

        /* Perform the TLS handshake
         */
        ret = session.handshake();
	std::cout << "Spot C" << std::endl;
        if (ret < 0)
	  {
